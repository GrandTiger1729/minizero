...
function usage {
	...
	echo "./fight.sh [BOARD_SIZE] [NUM_FIGHT] [TOTAL_GAMES] [FIGHT_NAME] [EDAX_LEVEL] [MINIZERO_SIMULATION] [START_ITER] [force/noforce]"
	exit
}
...
# some argument
FIGHT_RESULT_DIR="fight_results_Par_no_forward_Max${MINIZERO_SIMULATION}_vs_edax_${EDAX_LEVEL}_t_15"
...
echo "CUDA_VISIBLE_DEVICES=$1 gogui-twogtp -black "${BLACK_PROGRAM}" -white "${WHITE_PROGRAM}" -auto ${FORCE} -games ${TOTAL_GAMES} -sgffile ${SGF_PREFIX_NAME} -size ${BOARD_SIZE} -threads ${NUM_FIGHT} -verbose -alternate"

CUDA_VISIBLE_DEVICES=$1 gogui-twogtp -black "${BLACK_PROGRAM}" -white "${WHITE_PROGRAM}" -auto ${FORCE} -games ${TOTAL_GAMES} -sgffile ${SGF_PREFIX_NAME} -size ${BOARD_SIZE} -threads ${NUM_FIGHT} -alternate -verbose
