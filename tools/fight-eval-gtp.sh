...
function run_twogtp(){
    ...
    BLACK="$sp_executable_file -mode console_gtp -conf_file $CONF_FILE1 -conf_str "nn_file_name=$FOLDER1/model/$2""
    WHITE="$sp_executable_file -mode console_gtp -conf_file $CONF_FILE2 -conf_str "nn_file_name=$FOLDER2/model/$2""
    EVAL_FOLDER="${FOLDER1}/$NAME/${2:12:-3}"
    SGFFILE="${EVAL_FOLDER}/${2:12:-3}"
    if [ -f "${SGFFILE}-$((${GAMENUM}-1)).sgf" ] || [ ! -f "$FOLDER2/model/$2" ] || [ ! -f "$FOLDER1/model/$2" ] ; then
        return
    fi
    if [ -f "${SGFFILE}-$((${GAMENUM}-1)).txt" ] || [ ! -f "$FOLDER2/model/$2" ] || [ ! -f "$FOLDER1/model/$2" ] ; then
        return
    fi
    KOMI=0
