    ...
    ExtMove* generate_moves(const Position& pos, ExtMove* moveList, Bitboard target)
    {
        ...
        while (bb) {
            ...
            if constexpr (Pt != CANNON) {
                b = (Pt != PAWN ? attacks_bb<Pt>(from, pos.pieces()) : pawn_attacks_bb(Us, from)) & target;
            } else {
...
{
    assert(static_cast<bool>(pos.checkers()));
    ...
    if (pt == CANNON) {
        ...
        if (hurdle) {
            ...
            if (pt == PAWN)
                ...
                b = (attacks_bb<ROOK>(hurdleSq, pos.pieces()) & ~line_bb(checksq, hurdleSq) & ~pos.pieces()) | (attacks_bb<CANNON>(hurdleSq, pos.pieces()) & pos.pieces(~us));
            else
