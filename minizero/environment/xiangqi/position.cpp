...
{
    return std::string{static_cast<char>('a' + file_of(s)), static_cast<char>('0' + rank_of(s))};
}
Square square(std::string s)
{
    return make_square(File(s[0] - 'a'), Rank(s[1] - '0'));
}
...
void Position::destruct_until(const StateInfo* si)
{
    while (st != si) {
...
void Position::update_blockers() const
{
    ...
    Bitboard snipers = ((attacks_bb<ROOK>(ksq) & (pieces(ROOK) | pieces(CANNON) | pieces(KING))) | (attacks_bb<KNIGHT>(ksq) & pieces(KNIGHT))) & pieces(~c);
...
{
    return (pawn_attacks_to_bb(WHITE, s) & pieces(WHITE, PAWN)) | (pawn_attacks_to_bb(BLACK, s) & pieces(BLACK, PAWN)) | (attacks_bb<KNIGHT_TO>(s, occupied) & pieces(KNIGHT)) | (attacks_bb<ROOK>(s, occupied) & pieces(ROOK)) | (attacks_bb<CANNON>(s, occupied) & pieces(CANNON)) | (attacks_bb<BISHOP>(s, occupied) & pieces(BISHOP)) | (attacks_bb<ADVISOR>(s) & pieces(ADVISOR)) | (attacks_bb<KING>(s) & pieces(KING));
}
...
{
    return ((pawn_attacks_to_bb(c, s) & pieces(PAWN)) | (attacks_bb<KNIGHT_TO>(s, occupied) & pieces(KNIGHT)) | (attacks_bb<ROOK>(s, occupied) & pieces(KING, ROOK)) | (attacks_bb<CANNON>(s, occupied) & pieces(CANNON))) & pieces(c);
}
...
bool Position::pseudo_legal(const Move m) const
{
    ...
    if (type_of(pc) == PAWN)
        return static_cast<bool>(pawn_attacks_bb(us, from) & to);
    else if (type_of(pc) == CANNON && !capture(m))
        return static_cast<bool>(attacks_bb<ROOK>(from, pieces()) & to);
    else
        return static_cast<bool>(attacks_bb(type_of(pc), from, pieces()) & to);
}
...
bool Position::gives_check(Move m) const
{
    ...
    if (attacks_bb<ROOK>(ksq) & pieces(sideToMove, CANNON)) {
        return static_cast<bool>(checkers_to(sideToMove, ksq, (pieces() ^ from) | to) & ~square_bb(from));
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck)
{
    ...
    assert(givesCheck == static_cast<bool>(st->checkersBB));
...
uint16_t Position::chased(Color c)
{
    ...
    while (attackers) {
        ...
        if (blockers_for_king(sideToMove) & from)
            ...
            attacks &= (pieces(~sideToMove) ^ pieces(~sideToMove, KING, PAWN)) | (pieces(~sideToMove, PAWN) & HalfBB[sideToMove]);
            ...
            if (chase_legal(m)) {
                ...
                if (trueChase) {
                    ...
                    if (attackerType == type_of(piece_on(to))) {
                        sideToMove = ~sideToMove;
                        if ((attackerType == KNIGHT && ((between_bb(from, to) ^ to) & pieces())) || !chase_legal(make_move(to, from)))
                            chase |= (1 << idBoard[to]);
                        sideToMove = ~sideToMove;
                    } else {
                        chase |= (1 << idBoard[to]);
...
bool Position::rule_judge(Value& result, int ply) const
{
    ...
    int end = std::min(st->rule60 + std::max(0, st->check10[WHITE] - 10) + std::max(0, st->check10[BLACK] - 10),
                       st->pliesFromNull);
    ...
    if (end >= 4 && filter[st->key] >= 1) {
        ...
        for (int i = 4; i <= end; i += 2) {
            ...
            checkThem &= static_cast<bool>(stp->checkersBB);
            ...
            if (stp->key == st->key && (++cnt == 2 || ply > i)) {
                if (!checkThem && !checkUs) {
                    ...
                    Position rollback;
                    memcpy(reinterpret_cast<void*>(&rollback), (const void*)this, offsetof(Position, filter));
                ...
                // Catch false mates
                if (result == VALUE_DRAW || cnt == 2 || (filter[st->key] <= 1 && st->previous->key == stp->previous->key))
                    return true;
            ...
            if (i + 1 <= end)
                checkUs &= static_cast<bool>(stp->previous->checkersBB);
        }
    }
...
bool Position::pos_is_ok() const
{
    ...
    if ((sideToMove != WHITE && sideToMove != BLACK) || piece_on(square<KING>(WHITE)) != W_KING || piece_on(square<KING>(BLACK)) != B_KING)
        assert(0 && "pos_is_ok: Default");

    if (Fast)
        return true;

    if (pieceCount[W_KING] != 1 || pieceCount[B_KING] != 1 || checkers_to(sideToMove, square<KING>(~sideToMove)))
        assert(0 && "pos_is_ok: Kings");

    if ((pieces(WHITE, PAWN) & ~PawnBB[WHITE]) || (pieces(BLACK, PAWN) & ~PawnBB[BLACK]) || pieceCount[W_PAWN] > 5 || pieceCount[B_PAWN] > 5)
        assert(0 && "pos_is_ok: Pawns");

    if ((pieces(WHITE) & pieces(BLACK)) || (pieces(WHITE) | pieces(BLACK)) != pieces() || popcount(pieces(WHITE)) > 16 || popcount(pieces(BLACK)) > 16)
    ...
    for (Piece pc : Pieces)
        if (pieceCount[pc] != popcount(pieces(color_of(pc), type_of(pc))) || pieceCount[pc] != std::count(board, board + SQUARE_NB, pc))
