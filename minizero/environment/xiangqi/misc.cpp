...
//         Tie(std::streambuf* b, std::streambuf* l) : buf(b),
//                                                     logBuf(l) {}
...
//     class Logger {
//         Logger() : in(std::cin.rdbuf(), file.rdbuf()),
...
#define dot_ver2(n)                                \
    compiler += static_cast<char>('.');            \
    compiler += static_cast<char>('0' + (n) / 10); \
    compiler += static_cast<char>('0' + (n) % 10);
...
// {
//     int64_t n;
//     auto E = [&n](int64_t x) { return static_cast<double>(x) / n; };
...
//     for (int i = 0; i < MaxDebugSlots; ++i)
//         if ((n = correl[i][0])) {
//             double r = (E(correl[i][5]) - E(correl[i][1]) * E(correl[i][3])) / (sqrt(E(correl[i][2]) - sqr(E(correl[i][1]))) * sqrt(E(correl[i][4]) - sqr(E(correl[i][3]))));
...
void prefetch(reinterpret_cast<void*>)
{
}
...
{
#if defined(_MSC_VER)
    _mm_prefetch(reinterpret_cast<char*>(addr), _MM_HINT_T0);
#else
...
namespace WinProcGroup {
    ...
    static int best_node(size_t idx)
    {
        ...
        ptr = buffer = reinterpret_cast<SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX*>(malloc(returnLength));
    ...
    void bindThisThread(size_t idx)
    {
        ...
        if (!fun4 || !fun5) {
            ...
            GROUP_AFFINITY* affinity = reinterpret_cast<GROUP_AFFINITY*>(malloc(elements * sizeof(GROUP_AFFINITY)));
